...
DESCRIPTION = """\
This program compiles a C/C++ file with/without cs a number of times to get
some idea of cs speedup and overhead in the preprocessor and direct modes.
...
PHASES = [
    "without cs",
    "with cs, preprocessor mode, cache miss",
    "with cs, preprocessor mode, cache hit",
    "with cs, direct mode, cache miss",
    "with cs, direct mode, cache hit"]
...
def test(tmp_dir, options, compiler_args, source_file):
    ...
    environment = {"CS_CACHE_DIR": ccache_dir, "PATH": environ["PATH"]}
    environment["CS_COMPILERCHECK"] = options.compilercheck
    if options.compression:
        environment["CS_COMPRESS"] = "1"
    if options.hardlink:
        environment["CS_HARDLINK"] = "1"
    if options.nostats:
        environment["CS_NOSTATS"] = "1"
...
def main(argv):
    ...
    op.add_option(
        "--cs",
        help="location of cs (default: %s)" % DEFAULT_CCACHE)
    op.add_option(
        "--compilercheck",
    ...
    if "cs" in basename(realpath(compiler)):
        op.error(
            "%s seems to be a symlink to cs; please specify the path to"
