...
win32getshell(char *path)
{
	...
	if (!sh && getenv("CS_DETECT_SHEBANG")) {
		/* Detect shebang. */
		FILE *fp;
		fp = fopen(path, "r");
		if (fp) {
...
*/
int
execute_fd(char **argv, const char *path_stdout, int fd_stdout, const char *path_stderr, int fd_stderr, char **env_vars)
{
	pid_t pid;
	int status;
	...
	if (pid == 0) {
		if (path_stdout) {
                        ...
                        fd_stdout = open(path_stdout, O_WRONLY|O_CREAT|O_TRUNC|O_EXCL|O_BINARY, 0666);
                        if (fd_stdout == -1) {
                                exit(1);
                        }
		}
                ...
                if (path_stderr) {
                        ...
                        fd_stderr = open(path_stderr, O_WRONLY|O_CREAT|O_TRUNC|O_EXCL|O_BINARY, 0666);
                        if (fd_stderr == -1) {
                                exit(1);
                        }
                }
...
{
  return execute_fd(argv, path_stdout, 0, path_stderr, 0, NULL);
}
#endif
