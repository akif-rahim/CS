...
char const CACHEDIR_TAG[] =
	...
	"# This file is a cache directory tag created by cs.\n"
...
/*--------------------------------------------------------------------*/
/* Static data for the source files we have cached.  */
static struct hashtable *stashed_files = NULL;
static int stashed_file_count = 0;

/*
 * Reads the content of a file into mmapped memory and sets up the memory
 * for sharing using shm_open. If the same file is opened twice, the same
 * pointer will be returned for both.
 *
 * The intention is to allow another process to read the same file we read
 * with out doing disk-based copies, even if another process is likely to
 * change the file on disk in the meantime.
 *
 * Unline read_file, the SIZE_IN is not a hint, but must be correct. If
 * SIZE_IN is zero then the size will be determined with stat. SIZE_OUT
 * is set to the amount of data read, which may be smaller that SIZE_IN if
 * a read error occured (in which case errno will be set).
 */
bool
read_file_and_stash (const char* path, size_t size_in, char** data,
                     size_t *size_out)
{
  int fd, ret;
  size_t pos = 0, allocated;
  ...
  if (!stashed_files)
    stashed_files = create_hashtable(1000, hash_from_string, strings_equal);

  sf = hashtable_search(stashed_files, (char*)path);
  if (sf)
    {
      *data = sf->data;
      *size_out = sf->size;
  ...
  /* else, the file path has not been seen before.  */
  ...
  allocated = size_in + sizeof(*sf);

  fd = open(path, O_RDONLY | O_BINARY);
  if (fd == -1)
    return false;

  /* Create a new shared_memory object.  */
  share_name = format("/cs-saved_file.%d.%d", getpid(), stashed_file_count++);
  shmd = shm_open(share_name, O_CREAT | O_RDWR, 0600);
  if (shmd == -1 || ftruncate (shmd, allocated) == -1)
    {
      cc_log ("Could not create shared memory file");
      *data = NULL;
      if (shmd >= 0)
	close (shmd);
      return false;
  ...
  /* Create a mmapped area for the file. We don't do this using the
     built-in mmap file-read because then we couldn't use it as shared
     memory (we don't trust the disk files not to change). */
  sf = mmap(NULL, allocated, PROT_READ | PROT_WRITE, MAP_SHARED, shmd, 0);
  if (sf == MAP_FAILED)
    fatal("mmap unsuccessful: %s", strerror(errno));
  strncpy (sf->shm_name, share_name, sizeof(sf->shm_name));
  free(share_name);
  close(shmd);

  pos = 0;
  while (pos < size_in) {
      ret = read(fd, sf->data + pos, size_in - pos);
      if (ret == 0 || (ret == -1 && errno != EINTR)) {
	  break;
      }
      if (ret > 0) {
  ...
  if (msync(sf, allocated, MS_SYNC|MS_INVALIDATE) != 0)
    {
      cc_log("msync returned '%s'", strerror(errno));
      goto failed;
    }

  *data = sf->data;
  *size_out = sf->size = pos;
  hashtable_insert(stashed_files, x_strdup(path), sf);
  return true;

failed:
  shm_unlink(sf->shm_name);
  munmap(sf, allocated);
  *data = NULL;
  return false;
}
...
/*
 * As hash_file, but use an mmap shared file.
 */
bool
hash_stashed_file(struct mdfour *md, const char *fname)
{
  char *data;
  size_t size;

  if (!read_file_and_stash(fname, 0, &data, &size))
    return false;

  hash_buffer(md, data, size);
  return true;
}

/*
 * Find a file in shared mmapped memory.
 * If the file doesn't exist, read it.
 */
struct stashed_file *
find_stashed_file(const char *path)
{
  struct stashed_file *result = NULL;
  if (stashed_files)
    result = hashtable_search(stashed_files, (char*)path);

  if (result == NULL)
    {
      char *data;
      size_t size;
      if (read_file_and_stash (path, 0, &data, &size))
	result = hashtable_search(stashed_files, (char*)path);
    }
...
delete_stashed_files (void)
{
  ...
  iter = hashtable_iterator(stashed_files);
  do {
      struct stashed_file *sf = hashtable_iterator_value(iter);
      shm_unlink (sf->shm_name);
