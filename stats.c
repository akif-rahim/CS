...
extern char *secondary_config_path1, *secondary_config_path2;
...
static struct {
	...
	{ STATS_CACHEHIT_CPP, "cache hit (preprocessed)       ", NULL, FLAG_ALWAYS },
        { STATS_CACHEHIT_CLOUD, "cache hit (cloud)              ", NULL, FLAG_ALWAYS },
	...
	{ STATS_ERROR,        "cs internal error              ", NULL, 0 },
...
stats_summary(struct conf *conf)
{
	...
	printf("secondary config      (readonly)    %s\n",
	       secondary_config_path1 ? secondary_config_path1 : "");
	printf("tertiary config       (readonly)    %s\n",
	       secondary_config_path2 ? secondary_config_path2 : "");
