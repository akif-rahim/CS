...
TEST(conf_create)
{
	...
	CHECK_STR_EQ_FREE1(format("%s/.cscache", get_home_directory()),
	                   conf->cache_dir);
...
TEST(conf_read_valid_config)
{
	...
	CHECK(conf_read(conf, "cs.conf", &errmsg));
	CHECK(!errmsg);

	CHECK_STR_EQ_FREE1(format("/%s/foo/%s", user, user), conf->base_dir);
	CHECK_STR_EQ_FREE1(format("%s$/%s/.cscache", user, user), conf->cache_dir);
...
TEST(conf_read_with_missing_equal_sign)
{
	struct conf *conf = conf_create();
	char *errmsg;
	create_file("cs.conf", "no equal sign");
	CHECK(!conf_read(conf, "cs.conf", &errmsg));
	CHECK_STR_EQ_FREE2("cs.conf:1: missing equal sign",
	                   errmsg);
	conf_free(conf);
}
...
TEST(conf_read_with_bad_config_key)
{
	struct conf *conf = conf_create();
	char *errmsg;
	create_file("cs.conf", "# Comment\nfoo = bar");
	CHECK(!conf_read(conf, "cs.conf", &errmsg));
	CHECK_STR_EQ_FREE2("cs.conf:2: unknown configuration option \"foo\"",
	                   errmsg);
	conf_free(conf);
}
...
TEST(conf_read_invalid_bool)
{
	...
	create_file("cs.conf", "disable=");
	CHECK(!conf_read(conf, "cs.conf", &errmsg));
	CHECK_STR_EQ_FREE2("cs.conf:1: not a boolean value: \"\"",
	                   errmsg);

	create_file("cs.conf", "disable=foo");
	CHECK(!conf_read(conf, "cs.conf", &errmsg));
	CHECK_STR_EQ_FREE2("cs.conf:1: not a boolean value: \"foo\"",
	                   errmsg);
	conf_free(conf);
}
...
TEST(conf_read_invalid_env_string)
{
	struct conf *conf = conf_create();
	char *errmsg;
	create_file("cs.conf", "base_dir = ${foo");
	CHECK(!conf_read(conf, "cs.conf", &errmsg));
	CHECK_STR_EQ_FREE2("cs.conf:1: syntax error: missing '}' after \"foo\"",
	                   errmsg);
...
TEST(conf_read_invalid_size)
{
	struct conf *conf = conf_create();
	char *errmsg;
	create_file("cs.conf", "max_size = foo");
	CHECK(!conf_read(conf, "cs.conf", &errmsg));
	CHECK_STR_EQ_FREE2("cs.conf:1: invalid size: \"foo\"",
	                   errmsg);
	/* Other cases tested in test_util.c. */
	conf_free(conf);
}
...
TEST(conf_read_invalid_sloppiness)
{
	struct conf *conf = conf_create();
	char *errmsg;
	create_file("cs.conf", "sloppiness = file_macro, foo");
	CHECK(!conf_read(conf, "cs.conf", &errmsg));
	CHECK_STR_EQ_FREE2("cs.conf:1: unknown sloppiness: \"foo\"",
	                   errmsg);
	conf_free(conf);
}
...
TEST(conf_read_invalid_unsigned)
{
	...
	create_file("cs.conf", "max_files =");
	CHECK(!conf_read(conf, "cs.conf", &errmsg));
	CHECK_STR_EQ_FREE2("cs.conf:1: invalid unsigned integer: \"\"",
	                   errmsg);

	create_file("cs.conf", "max_files = -42");
	CHECK(!conf_read(conf, "cs.conf", &errmsg));
	CHECK_STR_EQ_FREE2("cs.conf:1: invalid unsigned integer: \"-42\"",
	                   errmsg);

	create_file("cs.conf", "max_files = foo");
	CHECK(!conf_read(conf, "cs.conf", &errmsg));
	CHECK_STR_EQ_FREE2("cs.conf:1: invalid unsigned integer: \"foo\"",
	                   errmsg);
...
TEST(verify_absolute_base_dir)
{
	...
	create_file("cs.conf", "base_dir = relative/path");
	CHECK(!conf_read(conf, "cs.conf", &errmsg));
	CHECK_STR_EQ_FREE2("cs.conf:1: not an absolute path: \"relative/path\"",
	                   errmsg);

	create_file("cs.conf", "base_dir =");
	CHECK(conf_read(conf, "cs.conf", &errmsg));
...
TEST(verify_dir_levels)
{
	...
	create_file("cs.conf", "cache_dir_levels = 0");
	CHECK(!conf_read(conf, "cs.conf", &errmsg));
	CHECK_STR_EQ_FREE2("cs.conf:1: cache directory levels must be between 1 and 8",
	                   errmsg);
	create_file("cs.conf", "cache_dir_levels = 9");
	CHECK(!conf_read(conf, "cs.conf", &errmsg));
	CHECK_STR_EQ_FREE2("cs.conf:1: cache directory levels must be between 1 and 8",
	                   errmsg);
...
TEST(conf_update_from_environment)
{
	...
	putenv("CS_COMPRESS=1");
	CHECK(conf_update_from_environment(conf, &errmsg));
	CHECK(conf->compression);

	x_unsetenv("CS_COMPRESS");
	putenv("CS_NOCOMPRESS=1");
	CHECK(conf_update_from_environment(conf, &errmsg));
	CHECK(!conf->compression);
...
TEST(conf_set_new_value)
{
	...
	create_file("cs.conf", "path = vanilla\n");
	CHECK(conf_set_value_in_file("cs.conf", "stats", "chocolate", &errmsg));
	data = read_text_file("cs.conf", 0);
	CHECK(data);
...
TEST(conf_set_existing_value)
{
	...
	create_file("cs.conf", "path = chocolate\nstats = chocolate\n");
	CHECK(conf_set_value_in_file("cs.conf", "path", "vanilla", &errmsg));
	data = read_text_file("cs.conf", 0);
	CHECK(data);
	CHECK_STR_EQ_FREE2("path = vanilla\nstats = chocolate\n", data);
}
